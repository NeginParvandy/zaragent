...
CHAT_BACKUP = CHAT_FILE.with_name(
    "chat_service.py.bak_before_intent_guard_phase1"
)

GUARD_BACKUP = GUARD_FILE.with_name(
    "intent_guard.py.bak_before_intent_guard_phase1"
)

MARKER = "# INTENT_GUARD_PHASE1"
...
def restore_files() -> None:
    if CHAT_BACKUP.exists():
        copy2(CHAT_BACKUP, CHAT_FILE)

    if GUARD_BACKUP.exists():
        copy2(GUARD_BACKUP, GUARD_FILE)
    elif GUARD_FILE.exists():
        GUARD_FILE.unlink()


GUARD_SOURCE = r'''from __future__ import annotations
        ...
        if not has_supported_domain:
            ...
            if has_ambiguous_action:
                return IntentDecision(
                    ...
                    status="AMBIGUOUS",
                    domain="UNKNOWN",
                    intent="MISSING_DOMAIN",
                    confidence=0.92,
                    reply=(
                        "منظور درخواست شما را با اطمینان کافی "
                        "تشخیص ندادم. لطفاً مشخص کنید درخواست "
                        "مربوط به غذا، مرخصی، مأموریت یا تردد است."
                    ),
                    suggestions=(
                        "برای فردا مرخصی ثبت کن",
                        "غذای این هفته را رزرو کن",
                        "تردد امروز را نشان بده",
                    ),
                )
...
def main() -> None:
    if not CHAT_FILE.exists():
        fail(
            f"Chat service was not found: {CHAT_FILE}"
        )

    original_chat = CHAT_FILE.read_text(
        encoding="utf-8"
    )

    if MARKER in original_chat:
        print(
            "SKIPPED: Intent Guard phase 1 "
    ...
    import_anchor = (
        "from app.application.services.hr_service "
        "import HrService\n"
    )

    if import_anchor not in updated_chat:
        restore_files()
        fail(
    ...
    updated_chat = updated_chat.replace(
        ...
        (
            import_anchor
            + "from app.application.services.intent_guard "
            "import IntentGuard\n"
        ),
        1,
    )
    ...
    if init_anchor not in updated_chat:
        restore_files()
        fail(
            "ChatService initialization anchor "
            "was not found."
        )
    ...
    if handle_anchor not in updated_chat:
        restore_files()
        fail(
            "Chat handle anchor was not found."
        )

    guard_block = '''        text = normalize_persian_text(message)
...
'''
    ...
    print("SUCCESS")
    print(f"Created: {GUARD_FILE}")
    print(f"Patched: {CHAT_FILE}")
    print(f"Backup: {CHAT_BACKUP}")
    print(
        "Intent Guard tests passed: "
